...
import numpy as np
import time
from utils.util import AverageMeter, eval_metrix, write_to_txt  # Assicurati che questa importazione sia corretta

# Definizione della funzione safe_save per torch.save
def safe_save(obj, filepath, max_retries=3, delay=0.5):
    """
    Salva l'oggetto 'obj' nel percorso 'filepath'. Se il file esiste già, 
    viene rimosso prima del salvataggio. In caso di PermissionError (ad esempio per file lock),
    attende per 'delay' secondi e riprova fino a 'max_retries' volte.
    """
    if os.path.exists(filepath):
        try:
            os.remove(filepath)
        except PermissionError as e:
            print(f"PermissionError nel rimuovere {filepath}. Attendo {delay} secondi e riprovo...")
            time.sleep(delay)
            try:
                os.remove(filepath)
            except Exception as e:
                print(f"Impossibile rimuovere {filepath}: {e}")
                raise e
    ...
    while attempt < max_retries:
        try:
            ...
            print(f"File salvato correttamente: {filepath}")
            return
        except PermissionError as e:
            attempt += 1
            print(f"PermissionError durante il salvataggio di {filepath}. Tentativo {attempt} di {max_retries}. Attendo {delay} secondi...")
            time.sleep(delay)
    raise PermissionError(f"Impossibile salvare il file {filepath} dopo {max_retries} tentativi.")

# Definizione della funzione safe_np_save per np.save
def safe_np_save(filepath, array, max_retries=3, delay=0.5):
    """
    Salva l'array 'array' nel percorso 'filepath'. Se il file esiste già, 
    viene rimosso prima del salvataggio. In caso di PermissionError (ad esempio per file lock),
    attende per 'delay' secondi e riprova fino a 'max_retries' volte.
    """
    if os.path.exists(filepath):
...
class AdaPINN(PINN):
    ...
    def adaptation_one_epoch(self, epoch, dataloader):
        ...
        for iter, (x1, x2, y1, y2) in enumerate(dataloader):
            ...
            debug_info = "[train] epoch:{} iter:{} data loss:{:.6f}, PDE loss:{:.6f}, physics loss:{:.6f}, total loss:{:.6f}".format(
                epoch, iter + 1, loss1, loss2, loss3, loss.item())
            if epoch < 3:
                self.logger.debug(debug_info)

            if (iter + 1) % 50 == 0:
                print("[epoch:{} iter:{}] data loss:{:.6f}, PDE loss:{:.6f}, physics loss:{:.6f}".format(
                    epoch, iter + 1, loss1, loss2, loss3))
    ...
    def Adaptation(self, trainloader, validloader=None, testloader=None):
        ...
        for e in range(1, self.args.adaptation_epochs + 1):
            ...
            info = '[Train] epoch:{}, data loss:{:.6f}, PDE loss:{:.6f}, physics loss:{:.6f}, total loss:{:.6f}'.format(
                e, loss1, loss2, loss3, loss1 + self.alpha * loss2 + self.beta * loss3)
            ...
            if valid_mse < min_valid_mse and testloader is not None:
                ...
                if self.args.save_folder is not None:
                    safe_np_save(os.path.join(self.args.save_folder, 'true_label.npy'), true_label)
                    safe_np_save(os.path.join(self.args.save_folder, 'pred_label.npy'), pred_label)
                ##################################################################################
        ...
        self.clear_logger()
        if self.args.save_folder is not None:
            save_path = os.path.join(self.args.save_folder, 'finetune model.pth')
            safe_save(self.best_model, save_path)
...
def get_args():
    ...
    # parametri specifici per AdaPINN
    parser.add_argument('--pretrain_model', type=str, default=None, help='Path del modello pre-addestrato')
    ...
    parser.add_argument('--target_batch', type=int, default=-1, choices=[-1, 0, 1, 2, 3, 4, 5],
                        help='XJTU dataset è diviso in 6 batch, e TJU in 3 batch. '
                             'Se target_data è XJTU, il range di target_batch è [-1,0,1,2,3,4,5];'
                             'Se target_data è TJU, il range di target_batch è [-1,0,1,2];'
                             'Altrimenti, ignora target_batch')
...
def one_adaptation_task(args, source, target, source_batch=-1, target_batch=-1):
    ...
    # test del modello pre-addestrato sul dominio target
    ...
    if args.log_dir is not None and args.save_folder is not None:
        ...
        info = 'Source only: {} -> {} | MSE: {:.8f}, MAE: {:.6f}, MAPE: {:.6f}, RMSE: {:.6f}'.format(
            source, target, MSE, MAE, MAPE, RMSE)
...
def FineTune_TJU2XJTU():
    ...
    for lr, sb, tb in zip(lrs, source_batchs, target_batchs):
        for experiment in range(10):
            ...
            setattr(args, 'save_folder', f'./result of reviewer/TJU-XJTU/batch{tb}/Experiment{experiment}')
...
def FineTune_XJTU2TJU():
    ...
    for lr, sb, tb in zip(lrs, source_batchs, target_batchs):
        for experiment in range(10):
            setattr(args, 'adaptation_lr', lr)
            setattr(args, 'log_dir', 'logging.txt')
            setattr(args, 'save_folder', f'./result of reviewer/XJTU-TJU/batch{tb}/Experiment{experiment}')
...
def FineTune_HUST2MIT():
    ...
    for experiment in range(10):
        ...
        setattr(args, 'log_dir', 'logging.txt')
        setattr(args, 'save_folder', f'./result of reviewer/HUST-MIT/Experiment{experiment}')
...
def FineTune_MIT2HUST():
    ...
    for experiment in range(10):
        ...
        setattr(args, 'log_dir', 'logging.txt')
        setattr(args, 'save_folder', f'./result of reviewer/MIT-HUST/Experiment{experiment}')
    ...
    for i, source in enumerate(datasets):
        for j, target in enumerate(datasets):
            ...
            for e in range(10):
                ...
                setattr(args, 'log_dir', 'logging.txt')
                setattr(args, 'save_folder', f'./result of reviewer/{source}-{target}/Experiment{e + 1}')
                one_adaptation_task(args, source=source, target=target, source_batch=sb, target_batch=tb)

if __name__ == '__main__':
    # Decommenta la funzione che vuoi eseguire, ad esempio:
    #FineTune()
    FineTune_MIT2HUST()
    #FineTune_HUST2MIT()
    #FineTune_TJU2XJTU()
