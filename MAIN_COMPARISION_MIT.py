...
def get_args():
    ...
    parser.add_argument('--warmup_epochs', type=int, default=30, help='numero di epoche di warmup')
    parser.add_argument('--warmup_lr', type=float, default=2e-3, help='learning rate di warmup')
    parser.add_argument('--lr', type=float, default=1e-2, help='learning rate')
    parser.add_argument('--final_lr', type=float, default=2e-4, help='final learning rate')
    parser.add_argument('--lr_F', type=float, default=1e-3, help='learning rate di F')

    # model related
    parser.add_argument('--u_layers_num', type=int, default=3, help='numero di layer per u')
    parser.add_argument('--u_hidden_dim', type=int, default=60, help='dimensione nascosta per u')
    parser.add_argument('--F_layers_num', type=int, default=3, help='numero di layer per F')
    parser.add_argument('--F_hidden_dim', type=int, default=60, help='dimensione nascosta per F')
    ...
    parser.add_argument('--log_dir', type=str, default='logging.txt', help='log file, se None non viene salvato')
    # Imposta il percorso di salvataggio: verrà creata la cartella "results of reviewer/MIT results"
    parser.add_argument('--save_folder', type=str, default='results of reviewer/MIT results', help='cartella di salvataggio')
...
if __name__ == '__main__':
    ...
    setattr(args, 'model', 'MLP')

    if args.data == 'MIT':
        # Esegui 10 esperimenti per il dataset MIT
        for e in range(10):
            # Aggiorna il percorso di salvataggio per ogni esperimento
            setattr(args, 'save_folder', os.path.join('./results of reviewer/MIT-MLP results', f'Experiment{e+1}'))
